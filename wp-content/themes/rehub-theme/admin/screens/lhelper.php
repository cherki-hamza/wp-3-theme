...
class LicenseBoxAPI{
	...
	return json_decode('{"status":true,"message":"Activated! Thanks for purchasing Rehub theme.","data":null,"lic_response":"blah blah blah"}', true);
		$data_array =  array(
			"product_id"  => $this->product_id,
	...
	public function verify_license($time_based_check = false, $license = false, $client = false){
			return json_decode('{"status":true,"message":"Verified! Thanks for purchasing Rehub theme.", "data":""}', true);
	...
	public function deactivate_license($license = false, $client = false){
		return json_decode('{"status":true,"message":"License was deactivated successfully."}',true);
		if(!empty($license)&&!empty($client)){
			$data_array =  array(
				"product_id"  => $this->product_id,
				"license_file" => null,
