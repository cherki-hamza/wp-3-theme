...
function RH_el_elementor_frontend( $element) {
    ...
    if ( $element->get_settings( 'rh_gsap' ) == 'true' ) {
        wp_enqueue_script('gsap');
        if($element->get_settings( 'rh_gsap_trigger_type' ) == 'custom' || empty($element->get_settings( 'rh_gsap_trigger_type' )) || $element->get_settings( 'rh_gsap_trigger_type' ) == 'batch'){
            wp_enqueue_script('scrolltrigger');
        }
...
function rh_el_custom_print_template($content, $widget){
    ...
    //Check z
    $content = "<# if ( settings.rh_gsap_z ) {
        #>
        <div class=\"rhforce3d\">" . $content . "</div>
        <# 
    }
    else {
        #>" . $content . "<# 
