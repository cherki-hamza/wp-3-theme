...
function wpsm_woocharts_shortcode( $atts, $content = null ) {
	...
	if($searchbtn){
		...
		echo '<div class="search-wrap"><button id="btn_search" class="btn-comp-search def_btn font90 pb10 pl15 pr15 pt10 cursorpointer"><i class="rhicon rhi-search"></i> '.esc_html__('Add more items', 'rehub-theme').'</button><input type="hidden" id="compare_search_data" data-posttype="'. esc_attr($posttype) .'" data-terms="'. esc_attr($terms) .'" data-taxonomy="'. esc_attr($taxonomy) .'"></div>';
	}
	if($disable){
