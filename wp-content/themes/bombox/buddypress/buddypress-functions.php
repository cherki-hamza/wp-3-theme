...
class BP_Legacy extends BP_Theme_Compat {
	...
	public function enqueue_styles() {
		...
		if ( ! is_rtl() && isset( $ltr['location'], $ltr['handle'] ) ) {
			wp_enqueue_style( $ltr['handle'], $ltr['location'], array(), boombox_get_assets_version(), 'screen' );
		...
		if ( is_rtl() ) {
			...
			if ( isset( $rtl['location'], $rtl['handle'] ) ) {
				...
				wp_enqueue_style( $rtl['handle'], $rtl['location'], array(), boombox_get_assets_version(), 'screen' );
		...
		if ( ! is_rtl() && isset( $theme['location'] ) ) {
			...
			wp_enqueue_style( $theme['handle'], $theme['location'], array(), boombox_get_assets_version(), 'screen' );
		...
		if ( is_rtl() ) {
			...
			if ( isset( $theme_rtl['location'] ) ) {
				...
				wp_enqueue_style( $theme_rtl['handle'], $theme_rtl['location'], array(), boombox_get_assets_version(), 'screen' );
	...
	public function enqueue_scripts() {
		...
		if ( isset( $asset['location'], $asset['handle'] ) ) {
			wp_enqueue_script( $asset['handle'], $asset['location'], bp_core_get_js_dependencies(), boombox_get_assets_version(), true );
		}
		...
		if ( bp_is_register_page() || ( function_exists( 'bp_is_user_settings_general' ) && bp_is_user_settings_general() ) ) {
			...
			// Enqueue script.
			wp_enqueue_script( $asset['handle'] . '-password-verify', $asset['location'], $dependencies, boombox_get_assets_version(), true );
		}
