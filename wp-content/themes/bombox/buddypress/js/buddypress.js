...
jq(document).ready( function() {
	...
	/* Scroll to the main menu current element */
	scrollToNavCurElement();
	...
	if( jq("#header-cover-image").length && jq(".bp-cover-image").length ) {
		window.bp.CoverImage.Attachment.on('change:url', function (data) {
			if (data.attributes.url) {
				jq( '#buddypress' ).removeClass( 'bbp-no-cover-img' );
			} else {
				jq( '#buddypress' ).addClass( 'bbp-no-cover-img' );
			}
		});
	}
	 ...
	 * Refresh Captcha
	 */
	jq( 'body' ).on( 'click', '.bp-auth-refresh-captcha', function(){
		var _this = jq( this ),
			_container = _this.closest( '.captcha-container'),
			_type = _this.data('action');

		boombox_bbp_refresh_captcha( _container, _type );
		return false;
	} );

	/**
	 * Load captcha for the first time
	 */
	jq( '.bp-auth-refresh-captcha' ).each( function(){
	 ...
	 * @param selector
	 */
	function boombox_bbp_refresh_captcha( selector, type ){
		selector.find( '.captcha').attr( 'src', BP_DTheme.captcha_file_url + '?' + Math.random() + '&type=' + type + '&w=207&h=33' ).end().removeClass('loading');
	}

	/**
	 * Move labels to placeholders
	 */
	jq('#profile-details-section .bbp-editfield').each( function(){
		var _this = jq( this),
			_label = _this.find( ' > label'),
			_text = _label.text().replace(/\s/g, '').replace('*', ' *'),
			_input = _this.find( 'input, textarea' );

		_input.attr( 'placeholder', _text );
	} );


    jq('body').on( 'click', '#bb-bp-btn-resend', function(){
    	var _this = jq( this ),
			_id = _this.data( 'id' ),
			_token = _this.data( 'token' ),
        	_status_node = _this.parent();

    	if( ! _id || ! _token ) {
    		return false;
		}

        _status_node.show().html( BP_DTheme.auth_loading_message );

    	jq.post(
    		ajaxurl,
            {
                action      : 'bb_bp_resend_activation_email',
                id   		: _id,
                token    	: _token
            },
			function( response ){
                _status_node.parents( '#sign-in' ).find( 'form' )[0].reset();
                _status_node.show().html( response.data.message );
			}
		);
...
/**
 * Scrolls to the main menu current element
 */
function scrollToNavCurElement() {
	var mainNavEl = jq("#buddypress .bbp-main-nav, #buddypress #object-nav");
	if(mainNavEl.length)
		mainNavEl.scrollLeft(jq("#buddypress .bbp-main-nav .current, #buddypress .bbp-main-nav .selected").offset().left - parseInt(jq('.bbp-container').css('padding-left')));
}

/*
* *** Gamify Tooltip Functionality ***
*/
(function ( jq ) {
	jq.fn.gfyTooltip = function( options ) {
		/* **Parameters** */
		var settings = jq.extend({
			defaultTooltipStyle: '', // Provides default tooltip style. Available values: tooltip-left, tooltip-right, tooltip-bottom, tooltip-top
			autoAdjustStyle: true, // property allowing to automatically adjust tooltip styles based on position
			closeOnContentHover: false, // If true, will close the tooltip content when hovering on it. Applies only for desktop.
			tooltipHMargin: 20, // tooltip margin from tooltip toggle element, applies for left/right tooltips
			tooltipVMargin: 20, // tooltip margin from tooltip toggle element, applies for bottom/top tooltips
			reserveHSpacing: 20, // tooltip horizontal space from window
			reserveVSpacing: 50 // tooltip vertical space from window
			// * All spacings are specified in pixel
		}, options );

		/* **Variables** */
		var curSel = jq(this);
		var gfyToggleTooltipSel = '.gfy-toggle-tooltip';
		var gfyTooltipSel = '.gfy-tooltip';

		if(curSel.length) {
			/* **Functions** */
			/* Remove class with regular expression
			 *  el      - element selector
			 *  regex   - regular expression
			 *  action  - if = 0, match only
			 *            if = 1, remove matched class
			 * */
			var processClassWithRegex = function(el, regex, action) {
			...
			/* Reset Tooltip */
			var resetTooltip = function(curEl) {
				var tooltip = curEl.find(gfyTooltipSel);
				var tooltipArrow = curEl.find(gfyTooltipSel + ' '+ '.tooltip-arrow');
				// Reset margins given during tooltip position adjustment
				tooltipArrow.removeAttr('style');
				tooltip.removeAttr('style');
				processClassWithRegex(tooltip, '^tooltip-', 1);
			}

			/* **Click Events** */
			var closeTooltips = function(event){
				if(!jq( event.target ).is(gfyToggleTooltipSel + ' *'))
					jq(gfyToggleTooltipSel).removeClass('active');
			}
			jq(document).on("click", 'body', function (e) {
				closeTooltips(e);
			});
			jq(document).on("touchend", 'body', function (e) {
				closeTooltips(e);
			});
			jq(gfyToggleTooltipSel).on('touchstart click',function(e){
				jq(gfyToggleTooltipSel).not(this).removeClass('active');
				jq(this).addClass('active');
			});

			/* **Close on Content Hover** */
			if(settings.closeOnContentHover) {
				var toolTipVisibleCl = 'gfy-tooltip-visible';

				var tooltipSelHover = function(tooltipEl) {
					tooltipEl.find(gfyTooltipSel)
						.mouseenter(function(e){
							var scopeCurEl = jq(this);
							setTimeout(function(){
								scopeCurEl.removeClass(toolTipVisibleCl);
							}, 55)
						})
				...
				curSel.addClass('close-on-content-hover');
				curSel
					.mouseenter(function(e){
						var scopeCurEl = jq(this);
						scopeCurEl.find(gfyTooltipSel).addClass(toolTipVisibleCl);
						setTimeout(function(){
							tooltipSelHover(scopeCurEl);
						}, 55)

					})
					.mouseleave(function(e){
						jq(this).find(gfyTooltipSel).removeClass(toolTipVisibleCl);
						curSel.find(gfyTooltipSel).off("mouseenter");
					});

			}

			/* **Default Tooltip Style** */
			if(settings.defaultTooltipStyle!='' && settings.defaultTooltipStyle.match('(left|right)')) {
				resetTooltip(curSel);
				curSel.find(gfyTooltipSel).addClass(settings.defaultTooltipStyle);
			}

			/* **Adjustment of Tooltip Styles** */
			/* Note: In case of top / bottom tooltips auto adjustment of styles should be done in order to fit screen*/
			if(settings.autoAdjustStyle || settings.defaultTooltipStyle.match('(top|bottom)')) {
				/* **Helper Functions** */
				/* Set Margins */
				var setMargins = function(curEl) {
					curEl.find(gfyTooltipSel +' ' +'.tooltip-inner').removeAttr('style');
					curEl.find(gfyTooltipSel+'.tooltip-left .tooltip-inner').css('margin-right',settings.tooltipHMargin + "px");
					curEl.find(gfyTooltipSel+'.tooltip-right .tooltip-inner').css('margin-left',settings.tooltipHMargin + "px");
					curEl.find(gfyTooltipSel+'.tooltip-top .tooltip-inner').css('margin-bottom',settings.tooltipVMargin + "px");
					curEl.find(gfyTooltipSel+'.tooltip-bottom .tooltip-inner').css('margin-top',settings.tooltipVMargin + "px");
				}
				/* Close all Tooltips */
				var closeAllTooltips = function(){
					jq('.gfy-toggle-tooltip').removeClass('active');
				}

				/* **Tooltip Processing** */
				var processTooltip = function(curEl) {
					// Reset
					resetTooltip(curEl);

					// Default Tooltip style setting
					if(settings.defaultTooltipStyle!='') {
						curEl.find(gfyTooltipSel).addClass(settings.defaultTooltipStyle);
					}

					// Options
					var reserveHSpacing = settings.reserveHSpacing;
					var reserveVSpacing = settings.reserveVSpacing;
					var tooltipHmargin = settings.tooltipHMargin;
					var tooltipVmargin = settings.tooltipVMargin;

					// Variables
					var tooltipToggleElWidth = curEl.outerWidth();
					var tooltipToggleElHeight = curEl.outerHeight();
					var tooltip = curEl.find(gfyTooltipSel);
					var tooltipArrow = curEl.find('.tooltip-arrow');

					var tooltipActualWidth = tooltip.outerWidth(true);
					var tooltipActualHeight = tooltip.outerHeight(true);
					var tooltipWidth = tooltipActualWidth + reserveHSpacing + tooltipHmargin;
					var tooltipHeight = tooltipActualHeight + reserveVSpacing + tooltipVmargin;

					var windowWidth = jq(window).width();
					var windowHeight = jq(window).height();
					var windowScrollTop = jq(window).scrollTop();
					var elScrollTop = curEl.offset().top;
					var elOffsetLeft = curEl.offset().left;
					var spaceFromRight = windowWidth - elOffsetLeft - tooltipToggleElWidth;
					var spaceFromTop = elScrollTop - windowScrollTop;
					var spaceFromBottom = windowHeight - spaceFromTop - tooltipToggleElHeight;

					// Tooltip style detection logic for fitting the screen
					if(settings.autoAdjustStyle) {
						processClassWithRegex(tooltip, '^tooltip-', 1);
						if (spaceFromRight > tooltipWidth) {// Check if there is available space from right
							if (spaceFromTop > tooltipHeight /2 && spaceFromBottom > tooltipHeight /2) // Toggle right tooltip when the space above and below is enough
								tooltip.addClass('tooltip-right');
							else {
								if(spaceFromBottom > tooltipHeight) // Toggle bottom tooltip when the space above is not enough
							...
							if (elOffsetLeft > tooltipWidth && spaceFromRight < tooltipWidth / 3) // Toggle left tooltip if there is available space from left and not available space from top
								tooltip.addClass('tooltip-left');
							else {
								if (spaceFromTop > tooltipHeight) // Toggle top tooltip there is available space from top
									tooltip.addClass('tooltip-top');
								else
									tooltip.addClass('tooltip-bottom'); // Toggle top tooltip there is no available space from top
							}
						}
					}

					// Adjustment of tooltip position in case of top/ bottom tooltips
					if(processClassWithRegex(tooltip, '(top|bottom)', 0)) {
						var tooltipOffsetLeft = tooltip.offset().left;
						var tooltipOffsetRight = windowWidth - tooltip.offset().left;
						var tooltipWidthWithoutMargin = tooltipActualWidth + reserveHSpacing;
						var arrowML;
						// If left part of the tooltip is cut
						if(tooltipOffsetLeft<0) {
							tooltip.css('margin-left', (Math.abs(tooltipOffsetLeft) + reserveHSpacing)+"px");
							arrowML = ( parseInt(tooltipArrow.css('margin-left')) - (Math.abs(tooltipOffsetLeft) + reserveHSpacing) )+"px";
							tooltipArrow.css('margin-left',arrowML);
						}
						// If right part of the tooltip is cut
						else {
							if(tooltipOffsetRight < tooltipWidth) {
								var mrg = tooltipOffsetRight - tooltipWidthWithoutMargin;
								tooltip.css('margin-left', mrg+"px");
								arrowML = (parseInt(tooltipArrow.css('margin-left')) - mrg)+"px";
								tooltipArrow.css('margin-left',arrowML);
							}
						}
				...
				var processTooltips = function() {
					curSel.each(function(e) {
						processTooltip(jq(this));
						setMargins(jq(this));
					});
				}
				jq(function(){
...
jq('.gfy-toggle-tooltip.tooltip-general').gfyTooltip({
});
jq('.gfy-toggle-tooltip.tooltip-bottom-style').gfyTooltip({
	defaultTooltipStyle: 'tooltip-bottom',
	autoAdjustStyle: false
});
jq('.gfy-toggle-tooltip.tooltip-top-style').gfyTooltip({
	defaultTooltipStyle: 'tooltip-top',
	autoAdjustStyle: false
});
jq('.gfy-toggle-tooltip.tooltip-left-style').gfyTooltip({
	defaultTooltipStyle: 'tooltip-left',
	autoAdjustStyle: false
});
jq('.gfy-toggle-tooltip.tooltip-right-style').gfyTooltip({
	defaultTooltipStyle: 'tooltip-right',
	autoAdjustStyle: false
});
jq('.gfy-toggle-tooltip.tooltip-minimal').gfyTooltip({
	defaultTooltipStyle: 'tooltip-bottom',
	closeOnContentHover: true,
	tooltipVMargin: 12,
	autoAdjustStyle: false
});

/*
 * *** Gamify Badge More Functionality ***
 */
function gfyMoreBadgeFunc() {
	if(jq('.gfy-badge-more-func').length) {
		/* Variables */
		var moreFuncWrapper = jq('.gfy-badge-more-func');
		var badgeCountAttr = moreFuncWrapper.attr('data-badge-count');
		var allBadgeCount = jq('.gfy-badge-more-func .the-badge').length;
		var executeFunc = moreFuncWrapper.attr('data-execute');
		var moreBadgeSel = '.gfy-badge-more-func .more-badge';

		/**
		 * Function for showing wrapper in case if Badge More Functionality shouldn't be applied
		 */
		var showMoreFuncWrapper = function () {
			moreFuncWrapper.addClass('item-show');
			setTimeout(function () {
				moreFuncWrapper.addClass('item-fade');
			}, 10);
		}

		if (allBadgeCount > badgeCountAttr) {

			/**
			 * Function for executing Badge More Functionality
			 */
			var executeMoreFunc = function () {
				if (typeof badgeCountAttr !== typeof undefined && badgeCountAttr !== false) {
					jq(function () {
						var badgeCount = parseInt(badgeCountAttr) - 1;
						var moreBadgeCount = allBadgeCount - badgeCountAttr;
						var moreBadge;
						if (moreBadgeCount > 99)
							moreBadge = '<div class="more-badge the-badge"><span class="gfy-icon-btn icon-lg gfy-icon-plus"></span></div>';
						else
							moreBadge = '<div class="more-badge the-badge"><span class="gfy-icon-btn icon-lg ">' + '<span class="more-badge-icon gfy-icon-plus"></span>' + '<span class="more-badge-txt">' + moreBadgeCount + '</span>' + '</span></div>';
						jq('.gfy-badge-more-func .the-badge:eq(' + badgeCount + ')').after(moreBadge);
						jq(moreBadgeSel).nextAll().addClass('item-hide menu-item');
						showMoreFuncWrapper();
					})
					jq(document).on('click', moreBadgeSel, function (e) {
						var moreBadgeEl = jq(moreBadgeSel);
						moreBadgeEl.nextAll().removeClass('item-hide');
						jq(this).addClass('item-hide');
						setTimeout(function () {
							moreBadgeEl.nextAll().addClass('item-fade');
						}, 10);
					})
				}
			}

			/* Execute Badge More Functionality based on provided option */
			switch (executeFunc) {
				case 'mobile':
					var gfyMobileCheck = setInterval(function () {
						if (jq('html.touchevents').length) {
							executeMoreFunc();
							clearInterval(gfyMobileCheck);
						}
					}, 100);

					var gfyDesktopCheck = setInterval(function () {
						if (jq('html.no-touchevents').length) {
							showMoreFuncWrapper();
							clearInterval(gfyDesktopCheck);
						}
					}, 100);
					break;
				case 'all':
 ...
 * *** Gamify Tabs ***
*/
function gfyTabs(target, el) {
	jq('.gfy-tabs .tabs-menu a').click(function(e){
		e.preventDefault();
		var tabId = jq(this).attr('href');
		var curEl = jq(tabId);
		var parEl = jq(this).parent();
 ...
 *  *** GFY popup ***
 **/
function closeGfyPopup() {
	jQuery('.gfy-popup-overlay').remove();
}