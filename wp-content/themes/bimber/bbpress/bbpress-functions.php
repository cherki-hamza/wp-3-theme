...
class G1_BBPress {
    ...
    public function enqueue_head_styles() {
        ...
        $uri .= 'css/' . bimber_get_css_theme_ver_directory() . '/styles/' . $stack . '/bbpress-' . $skin . '.min.css';

        // Enqueue global styles.
        wp_enqueue_style( 'bimber-bbpress', $uri, array(), $version );
        ...
        if ( !$bbp_load_css_conditionally || bbp_is_single_user() ) {
            wp_enqueue_style( 'bimber-bbpress-users-single', $uri . 'css/' . bimber_get_css_theme_ver_directory() . '/styles/' . $stack . '/bbpress-users-single-' . $skin . '.min.css', array(), $version );
